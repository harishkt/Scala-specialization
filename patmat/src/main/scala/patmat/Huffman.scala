...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith(_._2 < _._2).map{
      case(character,weight) =>  Leaf(character,weight)
  }
   ...
   */
    ...
    else (makeCodeTree(trees.head,trees.tail.head) +: trees.drop(2)).sortWith(weight(_) < weight(_))
  }
   ...
   */
    def until(condition: List[CodeTree] => Boolean, todo: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (condition(trees)) trees
    else until(condition,todo)(todo(trees))
  }
  ...
  type CodeContent = (Char, List[Bit])
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
      case Nil => List()
      case (x,y) :: rest if (x == char) => y
      case (_,_) :: rest => codeBits(rest)(char)
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      case Leaf(c: Char,_) => List( (c, List.empty[Bit]) )
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def appendFront(bit: Bit)(code: CodeContent): CodeContent = (code._1, bit :: code._2)
      a.map(appendFront(0)) :::  b.map(appendFront(1))
    }
