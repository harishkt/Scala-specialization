...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, move) #:: tail => {
      val newExplored = explored + block
      val newNeighbours = newNeighborsOnly(neighborsWithHistory(block,move),newExplored)

      newNeighbours ++ from(tail ++ newNeighbours,newExplored)
    }
  }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter {
    case (block,move) => (block.b1 == goal && block.b2 == goal)
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case Stream.Empty => List.empty
    case (block,move) #:: tail =>  move
  }
}
